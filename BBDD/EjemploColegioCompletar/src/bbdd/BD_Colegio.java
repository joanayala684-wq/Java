...
public class BD_Colegio extends BD_Conector{
	...
	public Alumno buscar_alumno (String dni) throws ErrorBaseDatos {
		...
		try{
			...
			if ( reg.next()){
				...
				al=new Alumno(reg.getString("dni"),reg.getString("nombre"),reg.getString("curso"),reg.getInt("matricula"),reg.getString("telefono"));
	...
	public ArrayList<Alumno> buscarAlumnoMat (int mat) throws ErrorBaseDatos {
		String cadenaSQL="SELECT * from alumnos WHERE dni='"+mat+"'";

		ArrayList<Alumno> alumnos= ArrayList<>;
		try{
			this.abrir();
			s=c.createStatement();
			reg=s.executeQuery(cadenaSQL);
			while ( reg.next()){
				// La fecha que se extrae de la bbdd es sql.Date, hay que transformarla a LocalDate
				alumnos.add(new Alumno(reg.getString("dni"),reg.getString("nombre"),reg.getString("curso"),reg.getInt("matricula"),reg.getString("telefono")));
